...
static const char * iot_data_type_names [] = {"Int8","UInt8","Int16","UInt16","Int32","UInt32","Int64","UInt64","Float32","Float64","Bool","String","Null","Array","Map","Vector","Pointer", "Multi"};
static const uint8_t iot_data_type_sizes [] = {1u, 1u, 2u, 2u, 4u, 4u, 8u, 8u, 4u, 8u, sizeof (bool), sizeof (char*), 0u };
...
static iot_data_value_base_t iot_data_bool_true = { .value.bl = true, .base.type = IOT_DATA_BOOL, .base.constant = true };
static iot_data_value_base_t iot_data_bool_false = { .value.bl = false, .base.type = IOT_DATA_BOOL, .base.constant = true };
static iot_data_value_base_t iot_data_null = { .base.type = IOT_DATA_NULL, .base.constant = true };
...
{
  return (type <= IOT_DATA_NULL) ? iot_data_type_sizes[type] : 0u;
}
  ...
  if (v1->type == v2->type)
  {
    switch (v1->type)
    {
      ...
      case IOT_DATA_ARRAY:
      {
        ...
        return  ((a1->length == a2->length) && (v1->sub_type == v2->sub_type) && ((a1->data == a2->data) || ((v1->hash == v2->hash) && (memcmp (a1->data, a2->data, a1->length * iot_data_type_sizes[a1->base.sub_type]) == 0))));
      }
...
{
  assert (data && cache && (cache->sub_type == IOT_DATA_MULTI));
  if (data->composed)
  {
    if (data->type == IOT_DATA_VECTOR)
    {
      ...
      while (iot_data_vector_iter_next (&iter))
      {
        iot_data_cache_add (cache, &(iter.vector->values[iter.index]));
      }
    }
    else // Map
    {
      ...
      while (iot_data_map_iter_next (&iter))
      {
        iot_data_cache_add (cache, &(iter.node->value));
        iot_data_cache_add (cache, &(iter.node->key));
      }
    }
  }
}
...
iot_data_t * iot_data_array_transform (const iot_data_t * array, iot_data_type_t type)
{
  ...
  if (len)
  {
    ...
    while (iot_data_array_iter_next (&iter))
    {
      ...
      if (iot_data_cast_val (element, ptr, array->sub_type, type))
      {
        ptr += esize;
        asize++;
      }
...
static void iot_data_vector_copy_to_vector (const iot_data_t * from, iot_data_t * to, iot_data_type_t type, uint32_t * index, bool recurse)
{
  iot_data_vector_iter_t iter;
  ...
  while (iot_data_vector_iter_next (&iter))
  {
    ...
    if (element)
    {
      if (recurse && (element->type == IOT_DATA_VECTOR))
      {
        iot_data_vector_copy_to_vector (element, to, type, index, true);
      }
      else
      {
        iot_data_t * val = iot_data_transform (element, type);
...
iot_data_t * iot_data_vector_to_vector (const iot_data_t * vector, iot_data_type_t type, bool recurse)
{
  ...
  iot_data_t * to = iot_data_alloc_vector (size);

  if (size)
  {
    uint32_t index = 0u;
    iot_data_vector_copy_to_vector (vector, to, type, &index, recurse);
...
extern uint32_t iot_data_array_size (const iot_data_t * array)
{
  ...
  return (arr->length * iot_data_type_sizes[arr->base.sub_type]);
}
...
void iot_data_map_add (iot_data_t * map, iot_data_t * key, iot_data_t * val)
{
  ...
  assert (key->type == map->sub_type || map->sub_type == IOT_DATA_MULTI);
...
{
  assert (iter);
  return (iter->index < iter->array->length) ? ((char*) (iter->array->data) + (iter->index) * iot_data_type_sizes[iter->array->base.sub_type]) : NULL;
}
...
iot_data_t * iot_data_vector_iter_replace_value (const iot_data_vector_iter_t * iter, iot_data_t * value)
{
  assert (iter);
  iot_data_t * res = NULL;
  iot_data_vector_t * vector = (iot_data_vector_t*) iter->vector;
  if (iter->index < vector->size)
  {
...
{
  return (node && node->key) ? node->colour : IOT_NODE_BLACK;
}

static inline void iot_node_set_colour (iot_node_t * node, iot_node_colour_t colour)
{
  if (node && node->key) node->colour = colour;
}
