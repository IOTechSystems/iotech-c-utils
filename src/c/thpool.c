...
struct iot_thpool_* iot_thpool_init(int num_threads){
	...
	thpool_p = (struct iot_thpool_*)malloc(sizeof(struct iot_thpool_));
	if (thpool_p == NULL){
		err("iot_thpool_init(): Could not allocate memory for thread pool\n");
		return NULL;
	}
	...
	if (jobqueue_init(&thpool_p->jobqueue) == -1){
		err("iot_thpool_init(): Could not allocate memory for job queue\n");
		free(thpool_p);
		return NULL;
	}
	...
	if (thpool_p->threads == NULL){
		err("iot_thpool_init(): Could not allocate memory for threads\n");
...
int iot_thpool_add_work(iot_thpool_* thpool_p, void (*function_p)(void*), void* arg_p){
	...
	if (newjob==NULL){
		err("iot_thpool_add_work(): Could not allocate memory for new job\n");
		return -1;
	}
