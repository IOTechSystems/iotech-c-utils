...
docker run ${DOCKER_RUN_FLAGS} --cap-add=SYS_NICE --rm -e "BRANCH=${BRANCH}" -e "TARGET=${TARGET}" -e "USER=${USER}" -e "SONAR=${SONAR}" -e "SONAR_AUTH_TOKEN=${SONAR_AUTH_TOKEN}" -v "$(pwd)/${ARCH}/${SYS}:/iotech-iot/${ARCH}" "iotech-iot-${SYS}-${RAND}:${VER}"
RET=$?
